...
var (
    ...
    BuildVersion   = "<<< filled in by build >>>" // 构建版本
    BuildDate      = "<<< filled in by build >>>" // 构建日期
    BuildCommitSha = "<<< filled in by build >>>" // 构建的git的commit id
)

/**
 * 得到指定环境变量的值，否则返回提供的默认值
 */
...
func getEnvBool(key string) (envValBool bool) {
    ...
    return // 这里是有BUG吗？怎么不返回envValBool？
}
...
func main() {
    ...
    // 密码可以在控制台输入，也可以先写在一个文件里面，这里解析出了密码
